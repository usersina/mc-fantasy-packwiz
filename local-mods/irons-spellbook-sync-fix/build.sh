...
MOD_ID="irons-spellbook-sync-fix"
MOD_VERSION="1.0.0"
MC_VERSION="1.21.1"
LOADER="neoforge"
...
if ! find "$RUNTIME_DIR/mods" -maxdepth 1 -type f -name 'irons_spellbooks-*.jar' -print -quit | grep -q .; then
  echo "ERROR: Iron's Spells jar is missing from $RUNTIME_DIR/mods"
  exit 1
fi
